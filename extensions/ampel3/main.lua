block.add(function()
local state=false
local function drawAmpel(painter,x,y,offset)
  red=pins[1+offset]
...
local function drawAmpelFus(painter,x,y,offset)
  red=pins[1+offset]
  green=pins[2+offset]
  painter:setPen(nil)
  painter:setBrush("black")
...
local function paintEvent(painter,color)
  painter:setPen(nil)
  ...
  if state then
    painter:setBrush("#dddd22")
    painter:drawRect(5.3,5.8,.4,.4)
    painter:drawRect(5.3,0.8,.4,.4)
  end
end
local function getState(pin)
  return state
end
  return {width=10,height=6,name="Ampel 3",pins={{0,1,"INPUT"},{0,2,"INPUT"},{0,3,"INPUT"},{0,5,"INPUT"},{0,6,"INPUT"},{11,1,"OUTPUT"}},category="other",description="Ampelsystem mit Fußgägerüberweg und Schaltern",paintEvent=paintEvent,getState=getState,onpress=onpress,onrelease=onrelease}
end)
